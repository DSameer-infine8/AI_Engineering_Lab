...
def basic_embeddings():
    ...
    print(f"Vector norm: {np.linalg.norm(single_embedding):.4f}")  #Numpy is used here to Normalize the vector to understand better
    '''
    the output of the Numpy here should be 1 means even if we have big and more docs/content all the vectors are normalized 
    and if it is not equal to 1 then that means the vectors are not normalized refering the embedding model dont support normalization 
    also indicating the embedding model used here is not good
    '''
